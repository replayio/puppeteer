...
function resolveExecutablePath(launcher) {
    // Use replay browser on supported platforms.
    if (process.platform == "linux") {
      const replayDir = process.env.RECORD_REPLAY_DIRECTORY || `${process.env.HOME}/.replay`;
      const executablePath = `${replayDir}/puppeteer/chrome-linux/chrome`;
      const missingText = fs.existsSync(executablePath)
            ? undefined
            : `Missing chrome executable, try "npm i @recordreplay/puppeteer"`;
